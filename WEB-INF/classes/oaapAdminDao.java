...
public class oaapAdminDao {  
  ...
  QuestionAnswerJavaBean qajb=new QuestionAnswerJavaBean();	
	...
	try{
      ...
      String query="insert into "+subject+"_"+difficulty+" values(?,?,?,?,?,?,?,?)";
...
System.out.println(sno);
   ...
   return status;
	...
	try{
	  ...
	  PreparedStatement preparedstatement=con.prepareStatement("update "+subject+"_"+difficulty+" set sno=?,question=?,a=?,b=?,c=?,d=?,answer=? where sno="+old_sno);
...
public int oaapQuestionDelete(String subject,String difficulty,int sno){  
	...
	try{
	  ...
	  String query="delete from "+subject+"_"+difficulty+" where sno=?";
